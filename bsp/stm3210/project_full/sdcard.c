...
{
    SD_Error errorstatus = SD_OK;
    u16 NumOfBlocks; 	/* block size is 1, so this is TotalNumOfBytes */
    u8  power = 0;
    u16 count = 0, restwords = 0, idx = 0;
    u32 *tempbuff = (u32 *)txpd;
    u32 response_r1;
    u16 bytes_remain;
	u16 cur_buf_remain;
	u32 scratch_u32;
	u8  *scratch_u8_src_ptr;
	u8  *scratch_u8_dst_ptr;
    u8 x;
	u16 ceiling_blocks; 

	NumOfBlocks = sizeof(txpd_t) + p->tot_len;
	ceiling_blocks = (NumOfBlocks <= 0x1FF) ? (NumOfBlocks & 0x1FF) : 0;

    /* Send CMD53 IO_RW_EXTENDED */
    SDIO_CmdInitStructure.SDIO_Argument = 0x80000000 |            /* Write */
                                            ((1 & 0x7) << 28) |  /* Function number */
                                            (0x1 << 27) |         /* Block mode */
    ...
    bytes_remain = p->tot_len;
    power = convert_from_bytes_to_power_of_two(1);

    SDIO_DataInitStructure.SDIO_DataTimeOut = SD_DATATIMEOUT;
    SDIO_DataInitStructure.SDIO_DataLength = NumOfBlocks;
     ...
     */
	// First we send out txpd header
	while (!(SDIO->STA & (SDIO_FLAG_TXUNDERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DATAEND | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_STBITERR)))
    {
        if (SDIO_GetFlagStatus(SDIO_FLAG_TXFIFOHE) != RESET)
        {
			SDIO_WriteData(*(tempbuff + 0));
			SDIO_WriteData(*(tempbuff + 1));
			SDIO_WriteData(*(tempbuff + 2));
			SDIO_WriteData(*(tempbuff + 3));
			SDIO_WriteData(*(tempbuff + 4));
			SDIO_WriteData(*(tempbuff + 5));
			SDIO_WriteData(*(tempbuff + 6));
			break;
        }
    }

	// Second we send out pbuf->payload
	// Assume p->total_len == T(p->len, p->next->len ...)
	tempbuff = (u32 *)p->payload; 
	cur_buf_remain = p->len;
    while (!(SDIO->STA & (SDIO_FLAG_TXUNDERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DATAEND | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_STBITERR)))
    {
        if (SDIO_GetFlagStatus(SDIO_FLAG_TXFIFOHE) != RESET)
        {
            if (cur_buf_remain >= SD_HALFFIFOBYTES)
            {
                ...
                cur_buf_remain -= SD_HALFFIFOBYTES;
            ...
            {
				/*
				 * The complexity here is that the size of
				 * each fragment is not guaranteed to be 4
				 * byte aligned, so we have to take care of
				 * the case when there is little tail on 
				 * current fragment, any idea to handle this?
				 */
                restwords = cur_buf_remain >> 2;
                if ((restwords << 2) != cur_buf_remain) {
					if (p->next != NULL) {
						/* 
						 * need to converage current head with
						 * the head of next fragment
						 */
						for (count = 0; count < restwords; count++) {
                			SDIO_WriteData(*(tempbuff + count));
            			}												  
						count = cur_buf_remain - ((restwords)<<2);
						bytes_remain -= (restwords)<<2;
						scratch_u8_dst_ptr = (u8 *)&scratch_u32;
						scratch_u8_src_ptr = (u8 *)(tempbuff + restwords);
						for (idx = 0; idx < count; idx++) {
							*scratch_u8_dst_ptr = *scratch_u8_src_ptr;
							scratch_u8_dst_ptr++;
							scratch_u8_src_ptr++;
						}
						p = p->next;
						count = 4 - count;
						if (p->len < count) {
							rt_kprintf("Error: tiny fragment in pbuf\n");
							return(errorstatus);
						}
						scratch_u8_src_ptr = (u8 *)p->payload;
						for (idx = 0; idx < count; idx++) {
							*scratch_u8_dst_ptr = *scratch_u8_src_ptr;
							scratch_u8_dst_ptr++;
							scratch_u8_src_ptr++;
						}
						SDIO_WriteData(scratch_u32);
						cur_buf_remain = p->len - count;
						tempbuff = (u32 *)((u8 *)p->payload + count);
						bytes_remain -= 4;
						continue;
					} else {
                		restwords++;
				...
				/* force break out if out of bucket */
				if (bytes_remain == 0) {
					break;
				}               
            }
...
SD_Error SDIO_WriteMultiBlock(u8 fn, u32 *writebuff, u16 BlockSize, u16 NumOfBlocks)
{
    ...
    u32 bytes_remain;
    u8 x;
	u16 ceiling_blocks = (NumOfBlocks <= 0x1FF) ? (NumOfBlocks & 0x1FF) : 0;
