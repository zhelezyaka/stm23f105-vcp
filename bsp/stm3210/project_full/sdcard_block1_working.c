...
static SD_Error SDIO_Func_Init(void);
...
SD_Error SDIO_Func_Init()
{
    ...
    tlen = sizeof(*cmd)+4;
    rb = (u8 *)rt_malloc(tlen);
    rb[0] = (sizeof(*cmd)+4) & 0xff;
    rb[1] = ((sizeof(*cmd)+4) >> 8) & 0xff;
    rb[2] = 1;
    rb[3] = 0;

    cmd = (cmd_header_t *)(&rb[4]);
    cmd->size = sizeof(*cmd);
    cmd->command = 0x00A9;
    cmd->seqnum = 1;
    cmd->result = 0;
...
void rt_hw_sdcard_init()
{
	...
	//if (SDIO_Func_Init() != SD_OK) {
	// 	rt_kprintf("Error initialize firmware function...\n");
	//}
}
