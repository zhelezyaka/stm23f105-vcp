...
// Event handler
struct Connection;
typedef void(adb_eventHandler)(struct Connection * connection, adb_eventType event, uint16_t length, uint8_t * data);
...
Connection_t * ADB_addConnection(const char * connectionString, uint8_t reconnect, adb_eventHandler * eventHandler);
